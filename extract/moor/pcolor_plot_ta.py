...
S = zfun.lowpass((s[1:,:] + s[:-1,:])/2,f='godin')
TH = zfun.lowpass((th[1:,:] + th[:-1,:])/2,f='godin')
U = zfun.lowpass((u[1:,:] + u[:-1,:])/2,f='godin')
V = zfun.lowpass((v[1:,:] + v[:-1,:])/2,f='godin')
# OX = (ox[1:,:] + ox[:-1,:])/2
...
ax.text(.05, .1, 'Tidally averaged salinity [g/kg]', c='k', weight='bold', transform=ax.transAxes)
...
ax.set_ylim(top=5)
ax.set_ylabel('Z [m]')
ax.text(.05, .1, 'Tidally averaged potential Temperature [deg C]', c='w', weight='bold', transform=ax.transAxes)
#ax.set_xlabel('Time [days from start of record]')

ax = fig.add_subplot(413)
cs = ax.pcolormesh(TT, ZZ, U, cmap=cm.balance, vmin=-2.5, vmax=2.5)
fig.colorbar(cs)
ax.set_ylim(top=5)
ax.set_ylabel('Z [m]')
ax.text(.05, .1, 'Tidally averaged u [m/s]', c='w', weight='bold', transform=ax.transAxes)
#ax.set_xlabel('Time [days from start of record]')

ax = fig.add_subplot(414)
cs = ax.pcolormesh(TT, ZZ, V, cmap=cm.balance, vmin=-0.25, vmax=0.25)
fig.colorbar(cs)
ax.set_ylim(top=5)
ax.set_ylabel('Z [m]')
ax.text(.05, .1, 'Tidally averaged v [m/s]', c='w', weight='bold', transform=ax.transAxes)
ax.set_xlabel('Time [days from start of record]')
