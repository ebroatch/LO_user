...
for i in range(len(sect_list)):
    ...
    axs[0].scatter(i,tef_df['pg_p'],color='tab:green', label='PG in',marker=plot_marker[i])
    axs[0].scatter(i,tef_df['stressdiv_p'],color='tab:blue', label='stressdiv',marker=plot_marker[i])
    axs[0].scatter(ot,tef_df['dudt_p']-tef_df['coriolis_p']-tef_df['pg_p']-tef_df['stressdiv_p'],color='k', label='residual (advection)')
