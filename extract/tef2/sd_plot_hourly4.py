...
out_dir = out_dir0 / ('sd_plots_hourly4_' + Ldir['ds0'] + '_' + Ldir['ds1'])
...
for i in range(len(sect_list)):
    ...
    if sect_name=='b3.p':
        ...
        axs[3].plot((ds2['time'].values[pad:-pad+1])[pad:-pad+1], qprism/1000, color='tab:purple', linewidth=lw)
...
axs[2].set_ylabel(r'$F_{R}$\n$[m^{3}s^{-1} g\ kg^{-1}]$')
axs[0].set_ylabel(r'$F_{E}$\n$[m^{3}s^{-1} g\ kg^{-1}]$')
axs[1].set_ylabel(r'$F_{T}$\n$[m^{3}s^{-1} g\ kg^{-1}]$')
axs[3].set_ylabel(r'$Q_{prism}$\n$[10^{3}\ m^{3}s^{-1}]$')
...
plt.savefig(out_dir / ('sd_plot_hourly4.png'))
plt.close()
pfun.end_plot()
