"""
Erin's modifications to make hourly instead of daily tef extraction
...
out_dir = out_dir0 / ('bulk_hourly_' + Ldir['ds0'] + '_' + Ldir['ds1'])
...
for snp in sect_list:
    ...
    for vn in ds.data_vars:
        if ('time' in ds[vn].coords) and ('sbins' in ds[vn].coords):
            ...
            TEF_lp[vn] = zfun.lowpass(ds[vn].values, f='godin')[pad:-pad+1, :] #no subsampling
            vn_list.append(vn)
    ...
    time_lp = ds.time.values[pad:-pad+1] #no subsampling
    sbins = ds.sbins.values
    #TEF_lp['qabs'] = zfun.lowpass(qabs, f='godin')[pad:-pad+1:24]
    TEF_lp['qabs'] = zfun.lowpass(qabs, f='godin')[pad:-pad+1] #no subsampling
