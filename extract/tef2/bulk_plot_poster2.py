...
out_dir = out_dir0 / ('bulk_poster_plots2_' + Ldir['ds0'] + '_' + Ldir['ds1'])
...
fig, [ax0,ax1,ax2,ax3] = plt.subplots(3, 1, sharex=True,figsize=(15,7.7),gridspec_kw={'height_ratios': [1,4,2,2]})
# fig = plt.figure()   
...
for i in range(len(sect_list)):
    ...
    ylab_dict = {'Qprism': r'$Q_{prism}\ [10^{3}\ m^{3}s^{-1}]$',
                'Q': r'$Q_{in}\ [10^{3}\ m^{3}s^{-1}]$',
                'salt': r'$s_{in}\ [g\ kg^{-1}]$',
...
plt.savefig(out_dir / ('bulk_plot_poster2.png'))
plt.close()
pfun.end_plot()
