...
import Lfun, zrfun, zfun

Ldir = Lfun.Lstart(gridname='cas6')
Ldir['collection_tag'] = 'c0'
Ldir['grid'] = Ldir['data'] / Ldir['gridname']
...
in_dir0 = Ldir['data'] / 'cas6_v0_live' / 'tef2'
in_dir = in_dir0 / 'extractions_2018.01.01_2018.12.31'
...
yd = dti.dayofyear
year = otdt[0].year
ax.plot(yd,(St_dict[sect_list[0]]-St_dict[sect_list[-1]])/dx,'-',color='k')
...
ax.text(.05,.05,r'(d) $\partial S/\partial x\ [g\ kg^{-1}\ km^{-1}]$',
...
fig.tight_layout()
fig.savefig(out_dir / 'dsdx_spring_neap.png',dpi=300)
