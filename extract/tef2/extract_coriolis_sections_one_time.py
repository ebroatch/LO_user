...
#get v values averaged to the u position
aa = ds.v.values.squeeze()
CC['v'] = (aa[:, sect_df.jrp, sect_df.irp]  + aa[:, sect_df.jrm, sect_df.irm] + aa[:, sect_df.jrp-1, sect_df.irp]  + aa[:, sect_df.jrm-1, sect_df.irm])/4 #average 4 v values around u grid
...
# get f values similar to zeta (2d on rho grid)
aa = ds.f.values.squeeze()
CC['f'] = (aa[sect_df.jrp, sect_df.irp]  + aa[sect_df.jrm, sect_df.irm])/2
...
ds1['h'] = (('p'), CC['h'])
ds1['dd'] = (('p'), CC['dd'])
ds1['v'] = (('time','z', 'p'), CC['v'].reshape(1,NZ,NP))
ds1['f'] = (('time','p'), CC['f'].reshape(1,NP))
