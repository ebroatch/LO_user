...
fig, [ax0,ax1,ax2,ax3,ax4] = plt.subplots(5, 1, sharex=True,figsize=(8,10),gridspec_kw={'height_ratios': [1,4,2,2,2]}) #CHANGE THIS FOR SPLIT INTO 2 PLOTS
# fig = plt.figure()   
...
for i in range(len(gctags)):
    ...
    # ax5.set_xlabel('Day')
    ax4.xaxis.set_major_formatter(mdates.DateFormatter('%j'))
...
# plt.tight_layout()
plt.savefig(out_dir / ('tef_variables_plot_multimodel_'+sect_choice+'.png'))
plt.close()
pfun.end_plot()

#PLOT OF QPRISM AND SALT FLUX
plt.close('all')
# pfun.start_plot(fs=fs, figsize=(21,10))
fs=14
pfun.start_plot(fs=fs)
...
for i in range(len(gctags)):
    ...
    ax5.plot(ot,tef_df['Q_p'].to_numpy()*(tef_df['salt_p'].to_numpy()-tef_df['salt_m'].to_numpy()), color=plot_color[i], linewidth=lw, label=silllens[i])
    ax5.grid(True)
    ax5.set_ylabel(ylab_dict['Qdeltas'])
    ax5.set_ylim(0,75)
...
# ax3.text(.02, .9, 'D', horizontalalignment='left', verticalalignment='top', transform=ax3.transAxes, fontsize=14, fontweight='bold')
# ax4.text(.02, .9, 'E', horizontalalignment='left', verticalalignment='top', transform=ax4.transAxes, fontsize=14, fontweight='bold')
ax5.text(.02, .95, 'B', horizontalalignment='left', verticalalignment='top', transform=ax5.transAxes, fontsize=14, fontweight='bold')
...
# ax0.set_title('Middle of sill b3')
# plt.tight_layout()
plt.savefig(out_dir / ('tef_saltflux_plot_multimodel_'+sect_choice+'.png'))
plt.close()
pfun.end_plot()
...
ax4.set_ylabel(ylab_dict['storage'])
ax4.set_xlabel(ylab_dict['Qprism'])
...
# ax4.set_ylim(0,70) #CHANGE YLIM AFTER CHECKING PLOT
# ax4.set_xlim(0,70)
