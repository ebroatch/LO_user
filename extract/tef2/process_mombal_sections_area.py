...
out_dir = out_dir0 / ('processed_mombal_area_' + Ldir['ds0'] + '_' + Ldir['ds1'])
...
for sn in sn_list:
    ...
    a = ds['dd'].to_numpy() * ds['DZ'].to_numpy()
    V['q'] = q
    V['a'] = a
    ot = ds['time'].to_numpy()
    ...
    # make arrays of property times area - to get area average
    AV = dict()
    for vn in V.keys():
        if vn == 'a':
            AV[vn] = a
    ...
    for tt in range(NT):
        ...
        for vn in AV.keys():
            ...
            if vn == 'q':
                ...
                #zi = zeta[tt,:].squeeze()
                zi = zeta[tt,:]
                zi[np.isnan(AV[vn][tt,0,:])] = np.nan # jx, if q = NaN, zi = NaN
                #ssh[tt] = zi.mean() #jx
                ...
                # also keep track of volume transport
                #qnet[tt] = XF.sum() #jx
                anet[tt] = np.nansum(XF) #jx
                
            # scipy.stats.binned_statistic(x, values, statistic='mean', bins=10, range=None)
            XF[np.isnan(XF)] = 0 #jx
            TEFa[vn][tt,:] = binned_statistic(sf, XF, statistic='sum', bins=NS, range=(S_low,S_hi)).statistic
    ...
    for vn in ['qnet','fnet','ssh','anet']:
        ds[vn] = (('time'), TEFa[vn])
    # for vn in vn_list + ['q','salt2']: #CHANGE THIS
    for vn in vn_list + ['q','a','dudt','stressdiv','pg','coriolis']: #CHANGE THIS
        ds[vn] = (('time','sbins'), TEFa[vn])
    # save it to NetCDF
