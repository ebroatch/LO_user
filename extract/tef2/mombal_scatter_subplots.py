...
stressdiv_mean = np.zeros((len(sect_list),len(gtagexlist)))
stressdiv_std = np.zeros((len(sect_list),len(gtagexlist)))
resid_mean = np.zeros((len(sect_list),len(gtagexlist)))
resid_std = np.zeros((len(sect_list),len(gtagexlist)))
LsLtratio = np.zeros(len(gtagexlist))
...
for gi in range(len(gtagexlist)):
    ...
    LsLtratio[gi]=silllen[gi]/TE['TE_spring']
    ...
    for i in range(len(sect_list)):
        ...
        pg_mean[i,gi] = np.mean(tef_df['pg_p'])
        pg_std[i,gi] = np.std(tef_df['pg_p'])
        stressdiv_mean[i,gi] = np.mean(tef_df['stressdiv_p'])
        stressdiv_std[i,gi] = np.std(tef_df['stressdiv_p'])
...
for i in range(len(sect_list)):
    axs[i].errorbar(LsLtratio,pg_mean[i,:],yerr=pg_std[i,:],color='tab:red', label='Pressure gradient',marker='o',markersize=10,lw=1,capsize=6,ls='None')
    axs[i].errorbar(LsLtratio,stressdiv_mean[i,:],yerr=stressdiv_std[i,:],color='tab:green', label='Stress divergence',marker='o',markersize=10,lw=1,capsize=6,ls='None')
    axs[i].errorbar(LsLtratio,resid_mean[i,:],yerr=resid_std[i,:],color='k', label='Residual (advection)',marker='o',markersize=10,lw=1,capsize=6,ls='None')
    axs[i].legend()
    axs[i].errorbar(LsLtratio,pg_mean[i,:],color='tab:red',marker='o',markersize=10,lw=1,capsize=6,alpha=0.5)
    axs[i].errorbar(LsLtratio,stressdiv_mean[i,:],color='tab:green',marker='o',markersize=10,lw=1,capsize=6,alpha=0.5)
    axs[i].errorbar(LsLtratio,resid_mean[i,:],color='k',lw=1,capsize=6,alpha=0.5)
    axs[i].grid(True)

axs[0].text(0.05,0.9,'b1',transform=axs[0].transAxes)
axs[1].text(0.05,0.9,'b3',transform=axs[0].transAxes)
axs[2].text(0.05,0.9,'b5',transform=axs[0].transAxes)
