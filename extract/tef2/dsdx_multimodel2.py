...
for gi in range(len(gtagexlist)):
    ...
    for i in range(len(sect_list)-1):
        axs[gi+1].plot(dti,(St_dict[sect_list[i]]-St_dict[sect_list[i+1]])/dxlist[i],'-',color=plot_color[i],label=sect_list[i]+'-'+sect_list[i+1]) #swap sign
    ...
    axs[gi+1].plot(dti,(St_dict[sect_list[0]]-St_dict[sect_list[4]])/(dxlist[0]+dxlist[1]+dxlist[2]+dxlist[3]),'--',color='k',label=sect_list[0]+'-'+sect_list[4]) #plot ds/dx across whole sill
...
fig.savefig(out_dir / 'dsdx_spring_neap_pairs_multimodel_new.png',dpi=300)
