...
# if Ldir['testing']:
#     sect_list = ['ss2.p']
sect_list = ['a1.p','b1.p','b3.p','b5.p']
sect_label = ['a1','b1','b3','b5']
...
fig, axs = plt.subplots(3, 1, sharex=True,figsize=(10,5.4),gridspec_kw={'height_ratios': [1,2,2]})
# fig = plt.figure()   
...
for i in range(len(sect_list)):
    ...
    axs[0].plot(ot,FR, linewidth=lw, label=sect_label[i])
    axs[2].plot(ot,FE, linewidth=lw, label=sect_label[i])
    axs[3].plot(ot,FT, linewidth=lw, label=sect_label[i])
...
axs[0].set_ylim(2e4,4e4)
axs[0].set_ylim(-1e4,1e4)
