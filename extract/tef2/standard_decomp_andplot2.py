...
for ext_fn in sect_list:
    ...
    # cb2.ax.set_yscale('linear')
    # cb3.ax.set_yscale('linear')
    # cb4.ax.set_yscale('linear')
    ...
    #ADD LATERAL/VERTICAL FOR EXCHANGE
    #CALCULATE MEAN U2, S2
    ...
    u1L = (np.sum(u1*dz,axis=1))/(H) #shape NT,NX
    u1V = u1 - np.expand_dims(u1L,axis=1) #shape NT,NZ,NX expand_dims for broadcasting
    s1L = (np.sum(s1*dz,axis=1))/(H) #shape NT,NX
    s1V = s1 - np.expand_dims(s1L,axis=1) #shape NT,NZ,NX expand_dims for broadcasting
    ...
    FEL = np.sum(u1L*s1L*H*dd, axis=1) #shape NT
    FEV = np.sum(u1V*s1V*dA, axis=(1,2)) #shape NT
    ...
    u2mean = xr.DataArray(np.sum(u2*dA2, axis=(1,2))/np.sum(dA2, axis=(1,2)), coords={'time':ds.time}, dims=['time'])
    s2mean = xr.DataArray(np.sum(s2*dA2, axis=(1,2))/np.sum(dA2, axis=(1,2)), coords={'time':ds.time}, dims=['time'])
    ...
    axs[1].set_title('Tidal')
    axs[1].grid(True)
    #axs[1].set_xlim(pd.Timestamp('2020-06-30'), pd.Timestamp('2020-07-02')) #to see tidal cycle zoom
    
    fig.suptitle(Ldir['sect_name']+' lateral and vertical components')
    plt.savefig(out_dir / (Ldir['sect_name'] + '_sdlv.png'))
    plt.close()
    ...
    # cb3.ax.set_yscale('linear')
    # cb4.ax.set_yscale('linear')
    # cb5.ax.set_yscale('linear')
    # cb6.ax.set_yscale('linear')
    # cb7.ax.set_yscale('linear')
    # cb8.ax.set_yscale('linear')
    ...
    # cb7.ax.set_yscale('linear')
    # cb8.ax.set_yscale('linear')
    # cb9.ax.set_yscale('linear')
    # cb10.ax.set_yscale('linear')
    # cb11.ax.set_yscale('linear')
    # cb12.ax.set_yscale('linear')
