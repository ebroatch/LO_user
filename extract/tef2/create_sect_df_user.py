...
Simpler plot than in the LO version
...
ax.pcolormesh(plon,plat,-h, vmin=-200, vmax=0, cmap='Spectral_r')
pfun.dar(ax)
#ax.axis([-127.5,-122,46.5,51])
# ax.text(.05,.95,Ldir['gridname'],transform=ax.transAxes,
#     fontweight='bold')
ax.set_title('TEF sections')
plt.show()
...
def plot_line(sn):
    ...
    #     fontweight='bold')
    ax.plot(x,y,'-',c='k', lw=2, alpha=alpha)
    ax.text(x[0],y[0],sn,c='k',ha='center',va='bottom',
        fontweight='bold')
    plt.draw()
