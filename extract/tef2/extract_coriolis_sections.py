...
out_dir = out_dir0 / ('extractions_coriolis_' + Ldir['ds0'] + '_' + Ldir['ds1'])
...
for ii in range(N):
    ...
    # use subprocesses
    cmd_list = ['python3', 'extract_coriolis_sections_one_time.py',
...
# zr = zrfun.get_z(hh, eta, S, only_rho=True)
# dzr = np.diff(zr, axis=0) # NOTE: this is packed (z,t,p)
# DZR = np.transpose(dzr, (1,0,2)) # packed (t,z,p)
