...
for i in range(3):
    ...
    ax1.plot(par1_out.Time/24, zfun.lowpass((par1_out/par1_out.sel(Time=0)).values*100, f='godin'), color=linecolor, label=silllenlabel)
    ...
    ax3.plot(par3_in.Time/24, zfun.lowpass((par3_in/par3_in.sel(Time=0)).values*100, f='godin'), color=linecolor, label=silllenlabel)
    print('plotted\n')
    sys.stdout.flush()
...
fn_fig = Ldir['LOo'] / 'plots' / 'tplot_rtbasins_tracker2_basic.png'
plt.savefig(fn_fig)
plt.close()
#plt.show()
