...
fig, axs = plt.subplots(3,2,figsize=(12,13),gridspec_kw={'height_ratios': [6,6,1]}, sharex=True)
...
for i in range(5):
    ...
    axs[0,0].plot(time_hours/24, zfun.lowpass(count_start_outup_stay_outup, f='godin'), '--', color=linecolor, label=silllenlabel+' in depth range') 
    ...
    if i==0:
        ...
        axs[2,0].plot(ot_hours_delta/24,Qprism.to_numpy(), color='tab:gray', linewidth=2)
        axs[2,1].plot(ot_hours_delta/24,Qprism.to_numpy(), color='tab:gray', linewidth=2)
        ...
        axs[2,0].pcolor(ot_hours_delta/24, axs[1,0].get_ylim(), np.tile(snbg,(2,1)), cmap='Greys', vmin=-0.5, vmax=1.75, alpha=0.3, linewidth=0, antialiased=True)
        axs[2,1].pcolor(ot_hours_delta/24, axs[1,1].get_ylim(), np.tile(snbg,(2,1)), cmap='Greys', vmin=-0.5, vmax=1.75, alpha=0.3, linewidth=0, antialiased=True)
        axs[2,0].grid(True)
        axs[2,1].grid(True)
...
axs[1,1].set_title('Inner basin below sill height')
axs[1,1].grid(True)
...
axs[0,1].set_title('Inner basin above sill height')
axs[0,1].grid(True)
...
axs[1,0].set_title('Outer basin below sill height')
axs[1,0].grid(True)
...
axs[0,0].set_title('Outer basin above sill height')
axs[0,0].grid(True)
...
axs[2,0].text(.02, .5, 'E', horizontalalignment='left', verticalalignment='center', transform=axs[2,0].transAxes, fontsize=14, fontweight='bold')
axs[2,1].text(.02, .5, 'F', horizontalalignment='left', verticalalignment='center', transform=axs[2,1].transAxes, fontsize=14, fontweight='bold')


fn_fig = Ldir['LOo'] / 'plots' / 'tplot_rtbasins_tracker2_layers_sn.png'
plt.savefig(fn_fig)
plt.close()
#plt.show()
