...
for i in range(5):
    ...
    # axs[1].fill_between(s_bin_centers_in,rt_smean_in.statistic-rt_std_in.statistic,rt_smean_in.statistic+rt_std_in.statistic,color=linecolor2,alpha=0.3)
    axs[1].plot(s_bin_centers_in,rt_smean_in.statistic,color=linecolor,label=silllenlabel)
    ...
    axs[0].plot(s_bin_centers_est,rt_smean_est.statistic,color=linecolor,label=silllenlabel)
...
axs[0].legend(loc='upper right',bbox_to_anchor=(0.05, 0.98))
axs[1].legend(loc='upper right',bbox_to_anchor=(0.05, 0.98))
...
fn_fig = Ldir['LOo'] / 'plots' / 'tplot_rtbasins_tracker2_sdist_regions2.png'
plt.savefig(fn_fig)
plt.close()
#plt.show()
