...
plt.close('all')
fig, axs = plt.subplots(2,3,figsize=(18,7),gridspec_kw={'height_ratios': [6,1]},sharex=True)
...
for i in range(5):
    ...
    axs[0,0].plot(time_hours/24, zfun.lowpass(count_est_strict, f='godin'), '--', color=linecolor, label=silllenlabel+' no reentry')
    axs[0,1].plot(time_hours/24, zfun.lowpass(count_insill, f='godin'), color=linecolor, label=silllenlabel)#+' total') 
    axs[0,1].plot(time_hours/24, zfun.lowpass(count_insill_strict, f='godin'), '--', color=linecolor, label=silllenlabel+' no reentry') 
    axs[0,2].plot(time_hours/24, zfun.lowpass(count_in, f='godin'), color=linecolor, label=silllenlabel)#+' total') 
    axs[0,2].plot(time_hours/24, zfun.lowpass(count_in_strict, f='godin'), '--', color=linecolor, label=silllenlabel+' no reentry') 
...
axs[0,2].xaxis.set_tick_params(labelbottom=True)
...
axs[0,2].text(.02, .02, 'C', horizontalalignment='left', verticalalignment='bottom', transform=axs[0,2].transAxes, fontsize=14, fontweight='bold')
axs[1,0].text(.02, .5, 'D', horizontalalignment='left', verticalalignment='center', transform=axs[1,0].transAxes, fontsize=14, fontweight='bold')
axs[1,1].text(.02, .5, 'E', horizontalalignment='left', verticalalignment='center', transform=axs[1,1].transAxes, fontsize=14, fontweight='bold')
axs[1,2].text(.02, .5, 'F', horizontalalignment='left', verticalalignment='center', transform=axs[1,2].transAxes, fontsize=14, fontweight='bold')


fn_fig = Ldir['LOo'] / 'plots' / 'tplot_rtbasins_tracker2_strict_sn2.png'
plt.savefig(fn_fig)
plt.close()
#plt.show()
