...
for i in range(5):
    ...
    z_bin_edges = np.arange(-200,0,25) #25m increments from -200 to 0, which works with the release layers #try to see if we can plot with positive depths
    ...
    axs[2].plot(-z_bin_centers_in,rt_zmean_in.statistic,color=linecolor,linewidth=2,label=silllenlabel)

    #insill
    rt_zmean_insill = stats.binned_statistic(z_start, rt_strict_days_insill, statistic=np.nanmean, bins=z_bin_edges, range=None)
    rt_std_insill = stats.binned_statistic(z_start, rt_strict_days_insill, statistic=np.nanstd, bins=z_bin_edges, range=None) #Try adding standard deviation as error
    z_bin_centers_insill = (rt_zmean_insill.bin_edges[:-1]+rt_zmean_insill.bin_edges[1:])/2 #use the subsampled bin edges for plotting
    axs[1].fill_between(-z_bin_centers_insill,rt_zmean_insill.statistic-rt_std_insill.statistic,rt_zmean_insill.statistic+rt_std_insill.statistic,color=linecolor2,alpha=0.3)
    axs[1].plot(-z_bin_centers_insill,rt_zmean_insill.statistic,color=linecolor,linewidth=2,label=silllenlabel)
    ...
    axs[0].plot(-z_bin_centers_est,rt_zmean_est.statistic,color=linecolor,linewidth=2,label=silllenlabel)
...
axs[2].set_xlabel('Release depth [m]')
axs[2].legend(loc='upper right')

axs[0].set_xlim(0,200) #x axis is the salinity range
...
fn_fig = Ldir['LOo'] / 'plots' / 'tplot_rtbasins_tracker2_zdist_regions.png'
plt.savefig(fn_fig)
plt.close()
#plt.show()
