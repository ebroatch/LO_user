...
def make_initial_info(gridname=gridname):
    ...
    if gridname == 'test0':
        ...
        # Erin new idealized model similar to Admiralty Inlet - long version
        # minimum depth 8m and land height 2m to try to eliminate blowups
        ...
        TL_side = 2000 # steepness of sides and end (transition length in m, approx 10% slope)
        # variable bathymetry parameters
        L_sill = 80000 # length of sill (length of flat section)
        HR_sill = 0.75 # height ratio of sills
        CR_sill = 0.5 # constriction ratio at sills
        # calculate additional constants
        L_estuary = (2*L_basin)+L_sill
    ...
    elif gridname == 'sill5km':
        # Erin new idealized model similar to Admiralty Inlet - short version
        # minimum depth 8m and land height 2m to try to eliminate blowups
        ...
        TL_side = 2000 # steepness of sides and end (transition length in m, approx 10% slope)
        # variable bathymetry parameters
        L_sill = 5000 # length of sill (length of flat section)
        HR_sill = 0.75 # height ratio of sills
        CR_sill = 0.5 # constriction ratio at sills
        # calculate additional constants
        L_estuary = (2*L_basin)+L_sill
        ...
        # Erin new idealized model similar to Admiralty Inlet - medium-long version
        # minimum depth 8m and land height 2m to try to eliminate blowups
        ...
        # create a river file by hand
        Ldir = Lfun.Lstart()
        dch['ctag'] = 'sill40km_base' #need to check what to call this
        ri_dir = Ldir['LOo'] / 'pre' / 'river1' / dch['ctag']
        Lfun.make_dir(ri_dir)
        gri_fn = ri_dir / 'river_info.csv'
        with open(gri_fn, 'w') as rf:
        ...
        # Erin new idealized model similar to Admiralty Inlet - medium-short version
        # minimum depth 8m and land height 2m to try to eliminate blowups
        ...
        # create a river file by hand
        Ldir = Lfun.Lstart()
        dch['ctag'] = 'sill10km_base' #need to check what to call this
        ri_dir = Ldir['LOo'] / 'pre' / 'river1' / dch['ctag']
        Lfun.make_dir(ri_dir)
        gri_fn = ri_dir / 'river_info.csv'
        with open(gri_fn, 'w') as rf:
