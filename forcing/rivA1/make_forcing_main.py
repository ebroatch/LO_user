"""
This is the main program for making the RIVER forcing file, for the updated ROMS.

Test on mac in ipython:

run make_forcing_main.py -g ae0 -r backfill -d 2020.01.01 -f rivA1 -test True
...
# use two points at start and end of day
dsf = Ldir['ds_fmt']
dt0 = datetime.strptime(Ldir['date_string'],dsf)
dt1 = datetime.strptime(Ldir['date_string'],dsf) + timedelta(days=1)
days = (dt0, dt1)
...
for vn in ['river_salt', 'river_temp']:
    ...
    if vn == 'river_salt':
        TS_mat = np.zeros((NT, N, NRIV)) #set river salinity to 0
    elif vn == 'river_temp':
        TS_mat = 10 * np.ones((NT, N, NRIV)) #set river temperature to 10C
    ds[vn] = (dims, TS_mat)
    ds[vn].attrs['long_name'] = vinfo['long_name']
    ds[vn].attrs['units'] = vinfo['units']
