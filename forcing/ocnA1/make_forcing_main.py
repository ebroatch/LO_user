...
z0 = zrfun.get_z(G['h'], G['h']*0, S, only_rho=True)
...
V['salt'] = 30*np.ones((NT, NZ, NR, NC)) - z0/50
V['salt'] = np.minimum(34*np.ones((NT, NZ, NR, NC)), V['salt'])
indest=zfun.find_nearest_ind(G['lon_rho'][0,:],0)+1 #find index of zero longitude which divides the estuary from the ocean
V['salt'][:,:,:,indest:]=0 #fill estuary with fresh water
